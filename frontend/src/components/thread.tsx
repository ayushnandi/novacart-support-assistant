...
/**
 * Component overrides for the thread.
 *
 * This app streams plain text only - the backend never returns attachments, images,
 * files, tool calls or reasoning parts - so the stock assistant-ui renderers for those
 * part types have been removed rather than left as unreachable code.
 *
 * `AssistantMessage` is required: `App.tsx` supplies `SupportAssistantMessage`, which
 * renders markdown plus the escalation badge and the thumbs up/down feedback bar.
 */
...
const useThreadComponents = (): ThreadComponents => {
  const components = useContext(ThreadComponentsContext);
  if (!components) throw new Error("Thread components are not available");
  return components;
};
...
const ThreadRoot: FC<{ isEmpty: boolean }> = ({ isEmpty }) => {
  const { Welcome = ThreadWelcome } = useThreadComponents();
...
const ThreadMessage: FC = () => {
  const { AssistantMessage } = useThreadComponents();
